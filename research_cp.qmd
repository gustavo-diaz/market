...
`r discipline` often study phenomena that cannot be observed directly. For example, we use responses to hypothetical survey questions to infer actual behavior, we resort to aggregate election results to understand individual evaluations of politicians' performance in office, and we conduct randomized controlled trials in some places to determine if a policy is advisable in other places. The phenomena we study remains hard-to-observe primarily due to informational imbalances among the actors we study. For example, citizens usually have an incomplete picture of what politicians do in office, and often react to new information by filling in the gaps.

My research focuses on how these informational imbalances present barriers to accountability, governance, and representation in `r region`. These are policy areas where governments, scholars, and civil society organizations have devoted considerable resources to the implementation of pro-democracy interventions, ranging from gender quotas to anti-corruption investigations. However, research in `r subfield` suggests these initiatives have a mixed record in terms of effectiveness. I aim to understand where, how, and why they work. 

I currently advance two research programs. The first focuses on the unintended consequences of investigating and disseminating information about the performance of elected officials. The second focuses on using public opinion surveys to document the extent and understand the nature of criminal governance in contexts where one would not expect this form of organized crime to emerge.
...
This program is evolving in two dimensions. First, the focus on unintended consequences of interventions aimed at overcoming the challenges to democracy has led me to focus on phenomena that constitute barriers themselves, yet remain hard to observe due to contextual challenges.

One such example is organized crime, where most of the conventional wisdom in Comparative Politics stems from studying countries where the scale of criminal governance is most pronounced (e.g. Brazil, Mexico). In a paper under **revise and resubmit** in *Latin American Politics and Society*, I document the prevalence of criminal governance strategies in Uruguay. This is a country with criminal organizations that are small in size but still deeply rooted in pockets of the capital city. Here, estimating the extent of exposure to criminal governance is challenging due to the combination of sensitivity bias, hard to reach populations, and our inability to oversample on key subgroups of interest. This means any attempt to maximize statistical precision needs to come from our research design choices. This is the basis of a recently awarded *European Research Council Starting Grant*  seeking to understand criminal governance in least-expected contexts from a comparative perspective, expanding to include comparative public opinion surveys in Argentina, Chile, and Costa Rica.

Second, this program also serves as a platform for innovation in research design and survey methods. For example, in a piece currently under review in *Political Analysis*, we use our pilot data to introduce methods to recover valid estimates when placebo-controlled list experiments produce implausible results, which helps in crafting question format and justifying questionnaire design. 

In another working paper, we combine list experiments and the network scale up method (NSUM), a popular technique in the health sciences, to improve precision in the estimation of the prevalence of sensitive attitudes and behaviors. Previous work combining different techniques to minimize sensitivity bias in survey questions relies on asking direct questions, but often these cannot be included in a survey for ethical or practical reasons. Our approach enables precision enhancements even when direct questions are not admissible.
