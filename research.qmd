...
fields = "design-based inference and computational social science"
applications = "to questions on the challenges to accountability, governance, and representation"
region = "Global South"
```
...
<!-- Two categories -->
My expertise has also translated into substantive contributions in a broad range of applications. I have primarily focused on issues of accountability, governance, and representation in the `r region`. These contributions fall into two categories.

<!-- Illuminate research practice -->
First, I have focused on projects that help illuminate survey experimental research practice. For example, in a manuscript forthcoming at the *British Journal of Political Science* with Virginia Oliveros (Tulane), Rebecca Weitz-Shapiro (Brown), and Matt Winters (Illinois), we use a survey experiment in Argentina to study gendered differential reactions to policy implementation. Previous work suggests that women face higher scrutiny for their performance in office. However, in the context of the implementation of a food distribution program, we find that voters are only responsive to performance information among men officeholders and tend to ignore performance information when told that an officeholder is a woman. We attribute this result to voters' perception of men politicians as the default category, thus providing no new information on top of performance. In turn, mentioning the gender of a woman politician leads voters to believe that good performance stems from factors beyond the incumbent's control. This has broader implications for research on gender gaps in the evaluation of public officials, since most survey-experimental analysis on the topic fail to account for this informational imbalance.

A second set of projects focuses on applying methodological innovations to improve statistical precision in areas where researchers need it the most. I am part of a recently awarded *European Research Council Starting Grant* seeking to understand criminal governance in least-expected contexts. Most of the conventional wisdom in Comparative Politics on this subject stems from studying countries where the scale of criminal governance is most pronounced (e.g. Brazil, Mexico). In a paper under **revise and resubmit** in *Latin American Politics and Society*, I document the prevalence of criminal governance strategies in Uruguay. This is a country with criminal organizations that are small in size but still deeply rooted in pockets of the capital city. Here, estimating the extent of exposure to criminal governance is challenging due to the combination of sensitivity bias, hard to reach populations, and our inability to oversample key subgroups of interest. This means any attempt to maximize statistical precision needs to come from our research design choices. This project is currently expanding to include comparative public opinion surveys Argentina, Chile, and Costa Rica.

This ERC grant also serves as a platform for innovation in research design and survey methods. For example, in a piece currently under review at *Political Analysis*, we use data from Uruguay to introduce methods to recover valid estimates when placebo-controlled list experiments produce implausible results. Placebo-controlled list experiments aim at reducing artificial inflation in prevalence estimates emerging from different list lengths across experimental conditions. However, they risk introducing attenuation bias if participants respond to the placebo item in unintended ways. As a solution, we introduce a partial identification approach that leverages list experimental assumptions and auxiliary information to recover how responses would have looked like without the placebo item. This helps in justifying questionnaire design choices at the planning stage.
